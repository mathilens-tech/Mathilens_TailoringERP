...
namespace MathilensERP.IntegrationTests.Customers;
...
public class CustomersEndpointsTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        var response = await _client.PostAsJsonAsync("/api/v1/customers", new { fullName = "Asha Rao", phoneNumber = "+91 98765 43210" });
    ...
    public async Task Create_WithBlankFullName_ReturnsValidationErrorEnvelope()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/v1/customers", new { fullName = "", phoneNumber = "+91 98765 43210" });
    ...
    [Fact]
    public async Task Create_WithBlankPhoneNumber_ReturnsValidationErrorEnvelope()
    {
        AuthenticateClient();

        var response = await _client.PostAsJsonAsync("/api/v1/customers", new { fullName = "Asha Rao", phoneNumber = "" });
    ...
    public async Task Create_WithMalformedEmail_ReturnsValidationErrorEnvelope()
    {
        ...
        var response = await _client.PostAsJsonAsync(
            "/api/v1/customers", new { fullName = "Asha Rao", phoneNumber = "+91 98765 43210", email = "not-an-email" });
    ...
    public async Task Search_WithPageBelowOne_ReturnsValidationErrorEnvelope()
    {
        ...
        var response = await _client.GetAsync("/api/v1/customers?page=0");
    ...
    public async Task Search_WithPageSizeAboveMaximum_ReturnsValidationErrorEnvelope()
    {
        ...
        var response = await _client.GetAsync("/api/v1/customers?pageSize=1000");
